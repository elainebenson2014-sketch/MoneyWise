...
.nav-mark{width:36px;height:36px;border-radius:9px;background:linear-gradient(135deg,#C9A84C,#E8C97A);display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0;color:#0D1F3C;font-weight:800;font-family:var(--serif)}
...
.tag-habit{background:rgba(201,168,76,0.2);color:#7A5C10}
...
const SYSTEM_PROMPT = `You are the MoneyWise Financial Coach — a warm, expert, encouraging personal finance coach.

Personality:
- Deeply encouraging, never shame-based or cold
- Practical, clear, and grounded in proven financial principles
- Teach one concept at a time — never overwhelm
- Celebrate every small win enthusiastically
- Inclusive and respectful of everyone's situation and values

Your core capabilities:
1. Build personalized budgets (50/30/20 or 10-10-80 give/save/live method)
...
6. Share practical money habits and tips

When someone shares their financial situation:
- Acknowledge their courage in facing their finances
- Identify the most urgent issue  
- Give ONE clear, actionable next step
- End with an encouraging, motivating word
...
async function askCoach(messages, userContext) {
  ...
  const systemPrompt = `You are the MoneyWise Financial Coach — a warm, expert, encouraging personal finance coach.
...
Be encouraging, practical, and warm. Give ONE clear actionable next step. Format with **bold** for key points.`;
  ...
  try {
    ...
    if (input.includes("budget") || input.includes("spend")) return "**For your budget**, start by tracking every dollar this week. A great framework is the 10-10-80 rule: Give 10%, Save 10%, Live on 80%. Even starting at 5-5-90 builds powerful momentum. Which expense category would you like to tackle first? 💰";
    ...
    if (input.includes("motivat") || input.includes("discourag")) return "**You are already ahead** — most people never face their finances honestly. Every consistent step forward compounds over time. Steady effort beats lucky timing — and you're already in motion. **This week:** Complete just ONE action from your dashboard. Momentum builds from small wins. 💪";
    if (input.includes("tip") || input.includes("habit") || input.includes("advice")) return "Here's a money habit worth building:\n\n**Pay yourself first.** Before you spend on anything else, move a set amount — even $25 — into savings the day you get paid. Automating it means you never have to rely on willpower. Over a year, small automatic transfers add up to real security. 🛡️";
  ...
  return {
    ...
    savingsGoals: (() => {
      ...
      if (goals.includes("give_more") || goals.includes("generational") || goals.length === 0)
        base.push({ name: "Giving Fund", target: Math.round(inc * 0.1 * 12), current: Math.round(sav * 0.15), icon: "💛" });
    ...
    actions: (() => {
      ...
      if (goals.includes("give_more"))
        acts.push({ text: `Set up a recurring $${Math.round(inc * 0.1)} monthly donation to a cause you care about`, tag: "savings" });
      ...
      acts.push({ text: "Pick one habit from the Weekly Habits tab and put it into practice this week", tag: "habit" });
      return acts.slice(0, 4);
    })(),
    scripture: { text: "Tell your money where to go before the month begins — a plan reduces stress more than a bigger paycheck does.", ref: "Habit of the Week" },
    encouragement: (() => {
      ...
      if (p === "spender") return `${name}, your generosity and enjoyment of life are wonderful qualities. Now let's channel that same energy into building something that lasts. Your future self is going to thank you for starting today.`;
      if (p === "saver") return `${name}, you already have the discipline — that's the hardest part. Now let's put that discipline to work at a higher level — smarter investing, bigger goals, and building wealth that lasts.`;
      ...
      return `${name}, the fact that you're here today is already an act of courage. Consistent, diligent effort is what builds real financial freedom — and your journey starts right now.`;
    })(),
    devotional: {
      day: "This Week",
      title: "Money as a Tool, Not a Master",
      body: "Money isn't meant to be a source of anxiety. It's a tool — to provide for your family, reach your goals, and build the life you want. Your budget isn't a cage; it's a roadmap to what matters most to you.",
      verse: "This week: write down your top 3 financial goals and keep them somewhere you'll see daily.",
    },
    lesson: {
...
const DEVOTIONALS = [
  { day: "Week 1 — Habit 1", title: "Money as a Tool, Not a Master", body: "Money isn't meant to be a source of anxiety. It's a tool — to provide for your family, reach your goals, and build the life you want. Your budget isn't a cage; it's a roadmap to what matters most to you.", verse: "This week: write down your top 3 financial goals and keep them somewhere you'll see daily." },
  { day: "Week 1 — Habit 2", title: "The Discipline of Delayed Gratification", body: "Financial discipline isn't deprivation — it's choosing your bigger goals over a smaller impulse. Every time you wait on a want, you say yes to a better tomorrow.", verse: "This week: pause 24 hours before any non-essential purchase over $50." },
  { day: "Week 1 — Habit 3", title: "Generosity Builds Wealth", body: "Counterintuitively, generous people often build more wealth — generosity keeps money in its proper place and guards against the fear of scarcity. Giving, in whatever form matters to you, is a healthy financial habit.", verse: "This week: decide on one cause or person you'd like to support, even a small amount." },
  { day: "Week 2 — Habit 1", title: "Plan Before You Spend", body: "When you tell your money where to go before the month starts, you stop wondering where it went. A simple plan reduces stress more than a bigger paycheck does.", verse: "This week: set a spending plan for your top 3 categories before payday." },
  { day: "Week 2 — Habit 2", title: "Contentment Over Comparison", body: "Comparison is the thief of joy and wealth. Social media fuels lifestyle inflation — buying things to keep up. Real wealth begins with gratitude for what you already have.", verse: "This week: unfollow one account that makes you want to spend." },
  { day: "Week 2 — Habit 3", title: "Debt Steals Your Future", body: "Debt borrows from your future income and limits your freedom. Paying it off isn't just smart math — it's buying back your options. Every dollar of debt eliminated is a dollar of freedom restored.", verse: "This week: make one extra payment, even $25, on your smallest debt." },
  { day: "Week 3 — Habit 1", title: "Track Honestly", body: "You can't improve what you don't measure. Honest tracking — every dollar in and out — turns vague money worry into clear, fixable numbers. Integrity with small amounts builds skill with larger ones.", verse: "This week: review and categorize the last 7 days of spending." },
  { day: "Week 3 — Habit 2", title: "Get Wise Counsel", body: "Don't make major financial decisions alone. Seek input — a trusted friend, a mentor, a financial professional. Pride says \"I've got this.\" Wisdom says \"help me think this through.\"", verse: "This week: talk through one money decision with someone you trust." },
  { day: "Week 3 — Habit 3", title: "Small Seeds, Big Harvest", body: "What you set aside today multiplies over years. Even small, consistent amounts saved or invested grow far more than you'd expect. Time is your greatest financial ally — so start now.", verse: "This week: set up one automatic transfer to savings, however small." },
  { day: "Week 4 — Habit 1", title: "Use What You Have", body: "You don't need to wait for more money to make progress. Take the resources you have, put them to work, and refuse to let fear keep you on the sidelines. Faithful, consistent effort wins.", verse: "This week: pick one account or goal and improve it by one concrete step." },
  { day: "Week 4 — Habit 2", title: "Give Yourself a Raise", body: "Automating savings is like giving your future self a raise. Setting aside even 10% before bills declares: my goals matter, not just my expenses.", verse: "This week: increase one automatic transfer by even 1%." },
  { day: "Week 4 — Habit 3", title: "Legacy Beyond Money", body: "Lasting wealth isn't only dollars — it's the wisdom, habits, and character you pass on. Teaching the people you love to work, save, and give is an inheritance that outlasts any balance.", verse: "This week: teach one money habit to someone in your family." },
];

const INSPIRATIONS = [
  { text: "Every dollar you save today is a vote for the life you want tomorrow.", ref: "MoneyWise" },
  { text: "Small, steady steps build the wealth that overnight luck never can.", ref: "MoneyWise" },
  { text: "You don't have to be wealthy to start — you have to start to become wealthy.", ref: "MoneyWise" },
  { text: "A budget isn't a limit on your freedom; it's the map that leads to it.", ref: "MoneyWise" },
  { text: "Progress, not perfection, is what turns goals into reality.", ref: "MoneyWise" },
  { text: "The best time to start was years ago. The second best time is today.", ref: "MoneyWise" },
  { text: "Wealth is built in the quiet discipline of everyday choices.", ref: "MoneyWise" },
  { text: "Pay yourself first, and your future will thank your present.", ref: "MoneyWise" },
  { text: "Calm doesn't come from a bigger paycheck — it comes from a plan.", ref: "MoneyWise" },
  { text: "You are always one good decision away from a stronger financial future.", ref: "MoneyWise" },
  { text: "Tell your money where to go, and you'll stop wondering where it went.", ref: "MoneyWise" },
  { text: "The habit you build this week is the wealth you keep next year.", ref: "MoneyWise" },
];

// Rotates each week so the saying feels fresh.
const weeklyInspiration = () => INSPIRATIONS[Math.floor(Date.now() / (7 * 864e5)) % INSPIRATIONS.length];
...
const LESSONS = [
  ...
  { badge: "Lesson 2", title: "The 10-10-80 Principle", body: "Give 10%, Save 10%, Live on 80%. This simple framework transforms your relationship with money from scarcity to intention. Start where you are and grow the percentages gradually over time.", tip: "💡 Even 5-5-90 is a powerful starting point. Growth happens one percentage at a time." },
  ...
  { badge: "Lesson 5", title: "The Power of Automation", body: "Decisions deplete willpower. Automation removes the decision. Set up automatic transfers: savings on payday, debt payment on payday, giving on payday. What you don't see, you don't spend.", tip: "💡 Pay yourself first — automatically." },
      ...
      if (savedPlan) {
        ...
        const restored = {
          ...
          savingsGoals: [
            ...
            { name: "Giving Fund", target: Math.round((savedPlan.income||0)*0.1*12), current: Math.round((savedPlan.savings||0)*0.15), icon: "💛" },
          ],
          actions: [
            ...
            { text: "Try one habit from the Weekly Habits tab today", tag: "habit" },
          ],
          scripture: { text: "Tell your money where to go before the month begins — a plan reduces stress more than a bigger paycheck does.", ref: "Habit of the Week" },
          ...
          devotional: { day:"This Week", title:"Staying the Course", body:"Financial transformation doesn't happen overnight — it happens one consistent decision at a time.", verse:"This week: keep going with one habit you've already started." },
  ...
  if (loadingSession) return (
    <>
      ...
      <div className="loading-page">
        ...
        <div className="loading-h">MoneyWise</div>
    ...
    <>
      ...
      <nav className="nav">
        <div className="nav-brand" onClick={() => setPage("landing")}>
          ...
          <span className="nav-name">Money<em>Wise</em></span>
        </div>
      ...
      {page === "dashboard" && (
        <ErrorBoundary onError={setAppError}>
          <Dashboard plan={plan || {income:0,expenses:0,debt:0,savings:0,surplus:0,totalAssets:0,incomeStreams:[],budget:[],debts:[],savingsGoals:[{name:"Emergency Fund",target:3000,current:0,icon:"🛡️"}],actions:[{text:"Review your spending this week",tag:"budget"}],scripture:{text:"Tell your money where to go before the month begins.",ref:"Habit of the Week"},encouragement:"Welcome! Complete your financial intake.",devotional:{day:"This Week",title:"Getting Started",body:"Every financial journey begins with a single step.",verse:"This week: write down your top 3 financial goals."},lesson:{title:"The Debt Snowball Method",body:"Pay minimums on all debts.",tip:"💡 Small wins build momentum."},user:{name:user?.name||"Friend",email:""},incomeStreams:[]}} user={user} dashTab={dashTab} setDashTab={setDashTab} checked={checked} setChecked={setChecked} checkinChecked={checkinChecked} setCheckinChecked={setCheckinChecked} onLogout={logout} onRedo={() => setPage("intake")} onPlanUpdate={(updated) => setPlan(updated)} />
        </ErrorBoundary>
      )}
    </>
  );
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        ...
        <div style={{ textAlign: "center", marginBottom: "1.5rem" }}>
          ...
          <p className="modal-sub">{isLogin ? "Sign in to MoneyWise" : "Join and receive your free financial plan"}</p>
        </div>
        {err && <div className="alert alert-err">{err}</div>}
    ...
    <div style={{ paddingTop:66, minHeight:"100vh", background:"#FAFAF6" }}>
      <div style={{ maxWidth:760, margin:"0 auto", padding:"3rem 2rem" }}>
        <div style={{ textAlign:"center", marginBottom:"2.5rem" }}>
          ...
          <p style={{ fontSize:"0.9rem", color:"#7A8BA8" }}>MoneyWise · Effective May 2026</p>
        </div>
        ...
        <div style={{ padding:"1rem 1.25rem", background:"#FDF7E8", border:"1px solid #E5D08A", borderRadius:10, marginBottom:"1.5rem", fontSize:"0.85rem", color:"#8B6914", lineHeight:1.7 }}>
          <strong>Important:</strong> MoneyWise is a financial education and coaching tool. It is NOT a licensed financial advisor, investment advisor, credit counselor, or tax professional. All content is for informational and educational purposes only.
        </div>
        ...
        {bullet("Household and personal preferences (household type, money personality, goals)")}
        ...
        {body("Financial outcomes depend on many factors including your income, expenses, discipline, and economic conditions. The debt-free projections and savings timelines shown are estimates based on the information you provide. MoneyWise makes no guarantees about specific financial outcomes.")}
        ...
        {bullet("Email: [your support email]")}
        {bullet("Website: [your website]")}
        ...
        <div style={{ marginTop:"2.5rem", padding:"1.25rem", background:"linear-gradient(135deg,#0D1F3C,#162E56)", borderRadius:12, textAlign:"center" }}>
          <p style={{ fontFamily:"Lora,Georgia,serif", fontSize:"0.9rem", fontStyle:"italic", color:"rgba(255,255,255,0.8)", lineHeight:1.7 }}>"A budget is telling your money where to go instead of wondering where it went."</p>
          <p style={{ fontSize:"0.75rem", color:"rgba(255,255,255,0.4)", marginTop:8 }}>MoneyWise · Effective May 2026</p>
        </div>
      </div>
    </div>
  );
...
function LandingPage({ onStart }) {
  const features = [
    { icon: "📊", title: "Budget Generator", desc: "Get a personalized budget built around your income and goals, using the simple 10-10-80 framework — give, save, and live intentionally." },
    ...
    { icon: "💰", title: "Savings Tracker", desc: "Set meaningful savings goals — emergency fund, big purchases, future plans — and watch your progress week by week with clear milestones." },
    { icon: "📖", title: "Weekly Money Habits", desc: "Practical, bite-sized habits and tips that connect smart financial principles to the real decisions you make every week." },
    ...
    <>
      <section className="hero">
        <div className="hero-inner">
          <div>
            <div className="hero-pill a1"><div className="pill-pulse" />Personal Finance, Made Simple</div>
            <h1 className="hero-h1 a2">Build <span className="g">Lasting</span><br />Wealth With<br />Clarity & Confidence.</h1>
            <p className="hero-lead a3">MoneyWise combines proven financial principles with AI-powered coaching to help individuals and families budget, reduce debt, save, and grow — one confident step at a time.</p>
            ...
            <div className="hero-trust a5">
              {["For Everyone", "AI-Powered", "Encouraging", "100% Free to Start"].map(t => (
            ...
            <div style={{ position: "relative" }}>
              <div className="hero-card">
                <div className="hero-card-header">
                  <span className="hc-title">Your Overview</span>
                ...
                <div className="hc-verse">
                  <p>"Tell your money where to go before the month begins — a plan beats a bigger paycheck."</p>
                  <cite>— MoneyWise Tip</cite>
                </div>
              </div>
      ...
      <section className="mission">
        <div className="mission-inner">
          <div className="mission-verse">"Wealth isn't built by what you earn in a moment — it's built by what you do, consistently, over time."</div>
          <div className="mission-ref">The MoneyWise Philosophy</div>
          <div className="mission-line" />
          <p className="mission-body">MoneyWise exists because financial freedom shouldn't be reserved for the few. When you manage your resources wisely, you reduce stress, provide for the people you love, and build a future with confidence. Everyone deserves the tools to get there.</p>
        </div>
      </section>
      ...
      <section className="sec" style={{ background: "white" }} id="features">
        <div className="sec-inner">
          <div className="sec-head">
            <div className="sec-eye">What's Included</div>
      ...
      <section style={{ background: "linear-gradient(135deg,#0D1F3C,#1B4D3C)", padding: "5rem 2rem", textAlign: "center", position: "relative", overflow: "hidden" }}>
        <div style={{ maxWidth: 600, margin: "0 auto", position: "relative" }}>
          ...
          <h2 style={{ fontFamily: "var(--serif)", fontSize: "2.6rem", fontWeight: 700, color: "white", marginBottom: "1rem", lineHeight: 1.2 }}>Your financial<br />journey starts today.</h2>
          <p style={{ fontSize: "1rem", color: "rgba(255,255,255,0.68)", marginBottom: "2.5rem", lineHeight: 1.85 }}>Complete your financial intake in 5 minutes and receive a personalized budget, debt strategy, savings goals, and encouraging coaching.</p>
          <button className="btn btn-gold btn-lg" onClick={onStart}>Create My Free Financial Plan →</button>
        </div>
      </section>

      <footer className="footer">
        <div className="footer-brand">MoneyWise</div>
        <div className="footer-copy">© 2026 MoneyWise · Smart money tools for everyone</div>
...
function IntakePage({ user, existingPlan, onComplete }) {
    ...
    <div style={{ minHeight:"100vh", paddingTop:66, background:"linear-gradient(180deg,#FAFAF6 0%,white 100%)", display:"flex", alignItems:"center", justifyContent:"center", padding:"100px 1.5rem 4rem" }}>
      <div style={{ maxWidth:440, width:"100%", background:"white", borderRadius:18, padding:"2.25rem", boxShadow:"0 20px 60px rgba(13,31,60,0.12)", border:"1px solid #E2EAF2" }}>
        <div style={{ textAlign:"center", marginBottom:"1.5rem" }}>
          ...
          <p style={{ fontSize:"0.85rem", color:"#7A8BA8", lineHeight:1.6 }}>Create a free account to save your MoneyWise plan and access it anytime.</p>
        </div>
  ...
  if (loading) return (
    <div className="loading-page">
      <div className="spinner" />
      <div className="loading-h">Building Your Plan…</div>
      <div className="loading-s">Crafting your budget, debt strategy, savings goals, weekly habit & first lesson.</div>
    </div>
  );
  ...
  const GOAL_OPTIONS = [
    ...
    { id:"give_more", icon:"❤️", label:"Give more generously", desc: totalInc > 0 ? `10% giving = $${Math.round(totalInc*0.1).toLocaleString()}/mo` : "10% of your income", connects:"Budget tab — highlights giving category" },
    ...
    { id:"reduce_stress", icon:"🌿", label:"Reduce financial stress", desc:"Get a clear plan and accountability", connects:"AI Coach — prioritizes encouragement & clarity" },
  ...
  const encouragement = moneyPersonality === "anxious" ? `${name||"Friend"}, take a deep breath. You don't have to figure everything out today. We'll take this one step at a time, together.`
    ...
    : `${name||"Friend"}, the fact that you're here is already an act of courage. Consistent, diligent effort is what builds real financial freedom — your journey starts right now.`;
        ...
        <div className="card card-p">
          ...
          {step === 4 && <>
            ...
            <div style={{ padding:"1.1rem", background:"#FDF7E8", border:"1px solid #E5D08A", borderRadius:10 }}>
              <div style={{ fontSize:"0.72rem", fontWeight:700, color:"#8B6914", marginBottom:5 }}>A word of encouragement</div>
              <div style={{ fontFamily:"Lora,Georgia,serif", fontSize:"0.88rem", fontStyle:"italic", color:"#7A5C10", lineHeight:1.72 }}>{encouragement}</div>
              <div style={{ fontSize:"0.72rem", color:"#C9A84C", fontWeight:700, marginTop:7 }}>Small, steady steps beat big, occasional efforts every single time.</div>
            </div>
          </>}
          ...
          <div className="intake-nav">
            ...
            {step < 4
              ...
              : <button className="btn btn-gold btn-lg" onClick={submit}>✨ Build My Plan</button>
            }
          </div>
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <div style={{ marginTop:"1rem", padding:"10px 14px", background:"#FDF7E8", border:"1px solid #E5D08A", borderRadius:8, fontSize:"0.78rem", color:"#8B6914", lineHeight:1.6 }}>
        💡 <em>"Small, steady steps build the wealth that overnight luck never can."</em> Every deposit, no matter how small, is a step toward freedom.
      </div>
    </div>
  );
}
...
function Dashboard({ plan, user, dashTab, setDashTab, checked, setChecked, checkinChecked, setCheckinChecked, onLogout, onRedo, onPlanUpdate }) {
  ...
  const sidebarItems = [
    ...
    { id: "devotional", icon: "📖", label: "Weekly Habits" },
    ...
    <div className="dash-layout" style={{ display: isMobile ? 'block' : 'flex', minHeight:'100vh', paddingTop: 66 }}>
      ...
      <aside className="sidebar" style={isMobile ? {
        ...
        <div className="sb-avatar">
          ...
          <div>
            ...
            <div className="sb-av-role">Member</div>
          </div>
        </div>
      ...
      <main className="dash-main" style={isMobile ? { marginLeft:0, padding:'1.4rem', width:'100%' } : {}}>
        <div style={{ marginBottom: "1.75rem" }}>
          ...
          <p className="dash-sub">{new Date().toLocaleDateString("en-US", { weekday: "long", year: "numeric", month: "long", day: "numeric" })} · Your financial dashboard</p>
        </div>
            ...
            <div className="dash-grid-2">
              ...
              <div className="dash-col">
                <div className="scripture-card">
                  <div className="scripture-eyebrow">✨ Inspiration</div>
                  <p className="scripture-text">"{weeklyInspiration().text}"</p>
                  <div className="scripture-ref">— {weeklyInspiration().ref}</div>
                </div>
          ...
          <div>
            <div style={{ marginBottom: "1.5rem" }}>
              <div className="scripture-card" style={{ padding: "2.25rem" }}>
                <div className="scripture-eyebrow">✨ Inspiration</div>
                <p style={{ fontFamily: "var(--serif)", fontSize: "1.5rem", fontStyle: "italic", color: "white", lineHeight: 1.7, marginBottom: "1rem", position: "relative" }}>"{weeklyInspiration().text}"</p>
                <div className="scripture-ref">— {weeklyInspiration().ref}</div>
              </div>
            </div>
          ...
          <div className="dash-grid-2">
            <div className="card">
              <div className="checkin-wrap">
                ...
                {["I tracked my spending this week", "I made my debt payment on time", "I moved money to savings", "I practiced one weekly money habit", "I set aside money for giving or a goal", "I stayed within my budget categories"].map((item, i) => (
            ...
            <div className="scripture-card" style={{ alignSelf: "start" }}>
              ...
              <p className="scripture-text">"Small wins, repeated, become big results."</p>
              <div className="scripture-ref">— Keep Going</div>
            </div>
          </div>
        )}
...
function AICoach({ user, plan }) {
  ...
  const hints = ["Walk me through my budget", "How do I stay motivated?", "Explain the snowball method", "Give me a money tip for today"];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "10px", padding: "1rem 1.25rem", background: "#0D1F3C", borderRadius: 12 }}>
        ...
        <div>
          <div style={{ fontWeight: 700, fontSize: "0.9rem", color: "white" }}>MoneyWise Coach</div>
      ...
      {score > 0 && range && (
        <>
          ...
          <div className="card card-p">
            ...
            <div style={{ marginTop:"1rem", padding:"10px 14px", background:"#FDF7E8", border:"1px solid #E5D08A", borderRadius:8, fontSize:"0.78rem", color:"#8B6914", lineHeight:1.6 }}>
              💡 A strong credit score is one of your most valuable financial tools — it lowers the cost of borrowing for years to come. Build it patiently and use it wisely.
            </div>
          </div>
        </>
      )}
...
const EXPENSE_CATEGORIES = [
  ...
  'Savings','Investments',
  'Giving / Donations','Charitable donations',
...
function BudgetTracker({ user }) {
  ...
  const generateAdvice = () => {
    ...
    const givingAmt = mExp.filter(r => r.cat==='Giving / Donations').reduce((s,r) => s+r.amt, 0);
    ...
    if (savPct >= 20) items.push({ icon:"⭐", text:`Outstanding! You saved ${savPct.toFixed(0)}% of income — above the 20% target. You're building real wealth.` });
    ...
    if (givingPct < 10) items.push({ icon:"❤️", text:`Giving was ${givingPct.toFixed(0)}% this month. The 10-10-80 framework suggests giving around 10% — but any amount counts, and even small steps build a generous habit.` });
    else items.push({ icon:"❤️", text:`You gave ${givingPct.toFixed(0)}% — generous and consistent. Giving is a healthy financial habit that keeps money in perspective.` });
  ...
  const annualStats = () => {
    ...
    aGiving=safeExpenses.filter(r=>r.key.startsWith(yearPrefix)).filter(r=>r.cat==='Giving / Donations').reduce((s,r)=>s+(parseFloat(r.amt)||0),0);
    ...
    <div style={{ fontFamily:'Nunito,sans-serif' }}>
      ...
      {tab === 'reconcile' && (() => {
        ...
        const guessCategory = (desc, isIncome) => {
          ...
          if (/(church|tithe|offering|ministry|donate|charity|nonprofit|fellowship|kingdom|gospel|missionary)/i.test(d)) return 'Giving / Donations';
...
function BudgetTab({ plan, user }) {
  ...
  const catKeywords = {
    ...
    'Giving': ['Tithe', 'Giving', 'Charity', 'Donation', 'Church'],
  ...
  return (
    <div className="card card-p">
      <div className="card-hdr" style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', flexWrap:'wrap', gap:8 }}>
        <div>
          <div className="card-title">Your Personalized Budget</div>
      ...
      {editing && (
        <div style={{ background:'#FDF7E8', border:'1px solid #E5D08A', borderRadius:10, padding:'1.25rem', marginBottom:'1.5rem' }}>
          ...
          <div style={{ display:'flex', gap:8, marginTop:'0.75rem' }}>
            <button onClick={()=>{
              setCustomPcts([
                { cat:'Giving', color:'#C9A84C', pct: 10 },
            ...
            }} style={{ background:'transparent', border:'1px solid #C9A84C', color:'#7A5C10', padding:'6px 12px', borderRadius:6, fontSize:'0.8rem', cursor:'pointer' }}>↩️ Reset to 10-10-80 Default</button>
      ...
      <div style={{ padding: "1.25rem", background: "#FDF7E8", border: "1px solid #E5D08A", borderRadius: 10 }}>
        <strong style={{ fontSize: "0.85rem", color: "#7A5C10" }}>💡 The 10-10-80 Principle:</strong>
